...
`)
  ...
  _waitForLastUpdateStatus(lambda, params, delay = 2, count = 0) {
    if (count >= 30) throw Error('asynchronous function update timed out.')
    if (params.LastUpdateStatus === 'Successful') return Promise.resolve()
    if (count === 0) console.log('=> Waiting for LastUpdateStatus')
    return new Promise(resolve => {
      setTimeout(resolve, delay * 1000)
    }).then(() => {
      return lambda.getFunction({ FunctionName: params.FunctionName }).promise()
        .catch(err => reject(err))
        .then(data => {
          if (data.Configuration.LastUpdateStatus === 'Failed')
            return reject(Error('asynchronous function update failure detected.'))
          if (data.Configuration.LastUpdateStatus === 'Successful') return
          return this._waitForLastUpdateStatus(lambda, data.Configuration, delay, count + 1)
        })
    })
  }
    ...
    return new Promise((resolve, reject) => {
      const updateConfigRequest = lambda.updateFunctionConfiguration(
        ...
        (err, configResponse) => {
          ...
          return this._waitForLastUpdateStatus(lambda, configResponse).then(() => {
            const updateCodeRequest = lambda.updateFunctionCode(functionCodeParams, (err, updateCodeResponse) => {
              if (err) return reject(err)
              resolve(updateCodeResponse)
            })
